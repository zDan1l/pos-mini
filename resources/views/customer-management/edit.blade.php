...
<div class="max-w-2xl mx-auto">
    <div class="mb-4">
        <a href="{{ route('customer-management.index') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-orange-500">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="p-6 border-b border-gray-100">
            ...
            <p class="text-sm text-gray-500 mt-1">Perbarui data customer</p>
        </div>
            ...
            <form id="editForm" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="w-32">
                            ...
                            @else
                                <div class="w-32 h-32 rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center">
                                    <i class="ph ph-user text-gray-400 text-3xl"></i>
                                </div>
                            @endif
                        </div>
                        ...
                        <!-- New Photo Preview -->
                        <div class="flex-1">
                            <p class="text-xs text-gray-500 mb-2">Foto Baru</p>
                            <div id="photoPreview" class="w-32 h-32 bg-gray-100 rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
                    ...
                    <div class="flex gap-2 mt-3">
                        <button type="button" id="btnOpenCamera" class="bg-orange-500 text-white py-2 px-4 rounded-lg hover:bg-orange-600 transition-colors text-sm flex items-center gap-2">
                            <i class="ph ph-camera"></i>
                            Ganti Foto
                        </button>
                        @if($customer->photo_path || $customer->photo_blob)
                            <button type="button" id="btnRemovePhoto" class="bg-red-100 text-red-600 py-2 px-4 rounded-lg hover:bg-red-200 transition-colors text-sm flex items-center gap-2">
...
<div id="cameraModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 overflow-hidden">
        <div class="p-4 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold">Ganti Foto Customer</h3>
...
@push('scripts')
    <script>
        ...
        btnRemovePhoto.addEventListener('click', () => {
            if (confirm('Hapus foto customer?')) {
        ...
        document.getElementById('editForm').addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('{{ route('customer-management.update', $customer->id) }}', {
                    method: 'PUT',
                    headers: {
                ...
                if (result.success) {
                    ...
                    btnSubmit.innerHTML = '<i class="ph ph-floppy-disk mr-2"></i> Simpan Perubahan';
                }
            } catch (err) {
