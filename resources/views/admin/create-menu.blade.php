...
<div class="mb-6">
    ...
    <p class="text-gray-600">Tambah menu untuk vendor</p>
</div>

<div class="max-w-2xl">
    <form method="POST" action="{{ route('admin.store-menu') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        @csrf
            ...
            <div>
                ...
                <select name="idvendor" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                    ...
                    @foreach($vendors as $vendor)
                        <option value="{{ $vendor->idvendor }}" {{ old('idvendor') == $vendor->idvendor ? 'selected' : '' }}>{{ $vendor->nama_vendor }}</option>
                    @endforeach
                </select>
                @error('idvendor')
