...
@section('content')
<div class="mb-6">
    <a href="{{ route('vendor.menus') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-orange-500">
        <i class="ph ph-arrow-left"></i>
        <span>Kembali ke Menu</span>
    </a>
    <h1 class="text-2xl font-bold text-gray-900 mt-4">Tambah Menu Baru</h1>
    <p class="text-gray-600">Tambah menu untuk {{ $vendor->nama_vendor }}</p>
</div>

<div class="max-w-2xl">
    <form method="POST" action="{{ route('vendor.store-menu') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="number" name="harga" required min="0" step="100"
                       class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                       placeholder="Contoh: 15000" value="{{ old('harga') }}">
                @error('harga')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-orange-500 transition-colors">
                    ...
                    <label for="menu-image" class="cursor-pointer">
                        ...
                        <p class="text-sm text-gray-400">JPEG, PNG, JPG (Maks 2MB)</p>
                    </label>
            ...
            <div>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_available" value="1" checked class="w-5 h-5 text-orange-500 border-gray-300 rounded focus:ring-orange-500">
        ...
        <div class="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-gray-200">
            <a href="{{ route('vendor.menus') }}" class="px-6 py-2.5 border border-gray-300 rounded-xl hover:bg-gray-50 transition-all font-medium">
                Batal
            </a>
