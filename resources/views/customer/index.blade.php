...
@section('content')
    <div class="max-w-7xl mx-auto">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                ...
                <a href="{{ route('customer.cart') }}" class="relative bg-orange-500 text-white px-4 py-2.5 rounded-xl hover:bg-orange-600 transition-colors flex items-center gap-2 flex-shrink-0">
                    ...
                    <span>Keranjang</span>
                    <span id="cart-badge" class="absolute -top-2 -right-2 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center {{ session('cart') ? '' : 'hidden' }}">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-6 overflow-hidden">
            <div class="flex items-center gap-1 p-1 overflow-x-auto" id="vendor-tabs">
                <button onclick="filterByVendor('all')" data-vendor="all"
                        class="vendor-tab active bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
                    <i class="ph ph-squares-four mr-1"></i>
                    Semua
                </button>
                @foreach($vendors as $vendor)
                    <button onclick="filterByVendor({{ $vendor->idvendor }})" data-vendor="{{ $vendor->idvendor }}"
                            class="vendor-tab text-gray-700 hover:bg-gray-100 px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
        ...
        <div id="menu-container" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            @foreach($vendors as $vendor)
                        ...
                        <div class="p-3">
                            ...
                            <div class="flex items-center justify-between mt-3">
                                ...
                                @if($menu->is_available)
                                    <button onclick="addToCart({{ $menu->idmenu }}, {{ $menu->idvendor }}, '{{ addslashes($menu->nama_menu) }}', {{ $menu->harga }})"
    ...
    <div id="modal-cart" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2 max-w-sm w-full mx-4 p-5">
    ...
    @push('scripts')
        <script>
            ...
            // Perbaikan: Format JS Modal Content
            function addToCart(idmenu, idvendor, namaMenu, harga) {
                const modalContent = document.getElementById('modal-content');
                // Menggunakan Intl.NumberFormat sebagai pengganti formatRupiah() di sisi client (JS)
                const formattedHarga = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(harga);
                
                modalContent.innerHTML = `
                    <p class="mb-2"><strong>${namaMenu}</strong> - ${formattedHarga}</p>
                    <input type="number" id="jumlah" value="1" min="1" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mb-4" placeholder="Jumlah">
            ...
            // Perbaikan: Fungsi updateCartBadge ditambahkan
            ...
            function showToast(message) {
                ...
                toast.className = 'fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm flex items-center';
                toast.innerHTML = `<i class="ph ph-check-circle mr-2 text-lg"></i>${message}`;
